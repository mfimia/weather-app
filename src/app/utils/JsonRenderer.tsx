...
const componentsMapping: Record<string, FC<JsonComponentProps>> = {
  header: ({ text }) => (
    <h1 className="text-3xl font-extrabold mb-4 text-blue-700">{text}</h1>
  ),
  text: ({ content }) => <p className="text-gray-600">{content}</p>,
  weatherInfo: ({ currentWeather }) =>
    currentWeather ? <WeatherInfo currentWeather={currentWeather} /> : null,
  daysForecast: ({ forecast }) =>
    forecast ? (
      <div className="mt-4 grid grid-cols-3 gap-4">
        {forecast.map((dayForecast) => (
          <DayForecast key={dayForecast.date} forecast={dayForecast} />
        ))}
      </div>
    ) : null
}
